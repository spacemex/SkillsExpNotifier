...
public class SkillExpNotifierFabricClient implements ClientModInitializer {

	public static final Path CONFIG_PATH = Path.of("config/SkillExpNotifier", "SkillExpNotifierConfig.yml");
	...
	public void onInitializeClient() {
		...
		HudRenderCallback.EVENT.register((drawContext, tickDelta) -> {
			NotifierClient.getToastComponent().render(drawContext);
		});

		ClientLoginHandler.register();
		...
		ClientPlayNetworking.registerGlobalReceiver(SkillExpNotifierFabric.XP_GAIN_PACKET_ID, (client, handler, buf, responseSender) -> {
			...
			client.execute(() -> {
				NotifierClient.getToastComponent().addToast(new XpToast(packet.categoryId(), packet.delta()));
			});
		});
	}

	private void genYAMLConfig(){
		File yamlFile = new File(CONFIG_PATH.toUri());
		yamlFile.getParentFile().mkdirs();
		YamlConfigTemplateWriter writer = new YamlConfigTemplateWriter(CONFIG_PATH.toFile());
		...
		writer
				...
				.add("Settings.Max-Toasts",3,"Maximum number of toasts to show at once")
